...
Start with main entrypoint to the generated app, `src/main.py`. This file defines an agent using LangChain_LangGraph and defines an entrypoint method with the Bedrock AgentCore SDK:
...
Next there is the `src/mcp_client` directory. Here you will find `client.py`. This file defines an MCP client from the chosen LangChain_LangGraph library. That client points to the
...
There is also a `.bedrock_agentcore.yaml` file output in the `latoagentcore` directory. This file contains a minimal definition with your agent name when it is created. After deploying the project, when running
